// id_decode.sv
// ID Stage: Decode + Register Read — ported from riscv-pipeline-cpu
// Adapted for 2-warp SIMT GPU core
...
module id_decode #(
    ...
    input  wire [$clog2(NUM_WARPS)-1:0] fetch_warp_in,

    // Register file
    output wire [4:0]  rs1_addr,
    output wire [4:0]  rs2_addr,
    output wire [$clog2(NUM_WARPS)-1:0] rf_warp_sel,
    ...
    output reg         jal_out,
    output reg  [$clog2(NUM_WARPS)-1:0] warp_id_out
);
    ...
    assign rs1_addr    = rs1;
    assign rs2_addr    = rs2;
    assign rf_warp_sel = fetch_warp_in;
